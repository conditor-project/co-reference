...
SourceManager.prototype.setPriorities = function (priorities) {
...
SourceManager.prototype.getSourcesOrderedByPriority = function () {
...
SourceManager.authorsEqual = function (firstAuthor, secondAuthor) {
...
SourceManager.prototype.addOrcIdInAuthors = function (authors) {
  for (const currentAuthor of authors) {
    ...
    if (currentAuthor.orcId || !currentAuthor.forename || !currentAuthor.surname) continue;
    sourcesLoop:
    for (const otherSource of this.getSourcesOrderedByPriority()) {
      for (const docObjectFromOtherSource of this.docObjects[otherSource]) {
        ...
        const authorsFromOtherSourceWithOrcId = _.filter(authorsFromOtherSource, 'orcId');
        ...
        for (const authorFromOtherSourceWithOrcId of authorsFromOtherSourceWithOrcId) {
          ...
          if (SourceManager.authorsEqual(authorFromOtherSourceWithOrcId, currentAuthor)) {
            currentAuthor.orcId = authorFromOtherSourceWithOrcId.orcId;
            break sourcesLoop;
          }
        }
      }
